...
for i = 1:num_static
    m = 1+(i-1)*winSize;
    for j = 1:winSize
        sum1 = a_static(m) +a_static(m + j -1);
    end
...
for i = 1:num_static
    m = 1+(i-1)*winSize;
    for j = 1:winSize
         sum2 = (a_static(m + j -1) - mean_static(i))^2;
    end
...
for i = 1:snum/winSize
    m = 1+(i-1)*winSize;
    for j = 1:winSize
         s_sum2 = (sample(m + j -1) - mean_static(i))^2;
    end
